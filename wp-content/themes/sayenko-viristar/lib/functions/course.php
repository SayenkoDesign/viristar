...
function vs_course_month_year($post_id = null)
{
    // Default date text
    $date = 'No Date';
    ...
    // Fetch the start date field
    $start = get_field('start_date', $post_id);

    // If no start date is provided, return default text
    if (empty($start)) {
        return $date;
    }
    ...
    // Format the start date to "Month, Year"
    $date = date('F, Y', $start_timestamp);
